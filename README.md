...
Registers a handler called once per FFT frame (~every 0.37 s) with the latest detection results. Inside this handler you can call `getNotePower(i)`, `getNoteCents(i)`, `getAvgNotePower()`, and `getMaxNotePower()` to build completely custom detection logic.
